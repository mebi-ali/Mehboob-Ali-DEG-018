...
def top__5_products():
    ...
    prod_sales= products_store.withColumn(
        "TotalSales", (products_store["Quantity"] * products_store["UnitPrice"]))

    return prod_sales.groupBy('Name').agg(F.sum(F.col('Quantity').cast(IntegerType())).alias('TotalQuantitiyPurchased')).orderBy(F.desc('TotalQuantitiyPurchased')).show(5)
